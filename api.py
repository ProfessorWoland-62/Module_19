...
class PetFriends:
    ...
    def get_list_of_pets(self, auth_key: json,
        ...
        питомцев, либо 'my_pets' - получить список
        собственных питомцев"""

        headers = {'auth_key': auth_key['key']}
        filter = {'filter': filter}

        res = requests.get(f'{self.base_url}api/pets',
                    headers=headers,
                    params=filter)
    ...
    def add_new_pet(self, auth_key: json,
        ...
        """Метод отправляет (постит) на сервер данные о добавляемом
        питомце и возвращает статус запроса на сервер и результат
        ...
        res = requests.post(f'{self.base_url}api/pets',
                    headers=headers,
                    data=data)
        status = res.status_code
        result = ''
    ...
    def delete_pet(self, auth_key: json,
        ...
        в формате JSON с текстом уведомления о успешном удалении.

        На сегодняшний день тут есть баг - в result приходит пустая
        строка, но status при этом = 200"""

        headers = {'auth_key': auth_key['key']}

        res = requests.delete(f'{self.base_url}api/pets/{pet_id}',
                    headers=headers)
        status = res.status_code
        result = ''
        ...
        try:
            ...
            print('\nНиже напечатан ответ на запрос delete', result)
            return status, result


    def update_pet_info(self, auth_key: json,
            pet_id: str,
            name: str,
            animal_type: str,
            age: int) -> json:
        ...
        и result в формате JSON с обновлёнными данными питомца"""
...
################## My API Methods #################
    ...
    def add_new_pet_without_photo(self, auth_key: json,
            name: str,
            animal_type: str,
            age: str) -> json:
        """Метод отправляет (постит) на сервер данные о добавляемом
        ...
        res = requests.post(f'{self.base_url}api/create_pet_simple',
                    headers=headers,
                    data=data)
    ...
    def add_pet_photo(self, auth_key: json,
            pet_id: str,
            pet_photo: str) -> json:
        """Метод отправляет (постит) на сервер фото питомца по
        указанному ID и возвращает статус запроса на сервер
