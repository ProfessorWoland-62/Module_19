...
def test_get_all_pets_with_valid_key(filter=''):
    ...
    Для этого сначала получаем api ключ и сохраняем
    в переменную auth_key. Далее, используя этот ключ,
...
def test_add_pet_photo(pet_photo='images/cat2.jpg'):
    ...
    if not my_pets['pets'] or 'data:image/jpeg;base64' in my_pets['pets'][0]['pet_photo']:
        pf.add_new_pet_without_photo(auth_key, "СуперПуперКот", "котяра", "10")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
